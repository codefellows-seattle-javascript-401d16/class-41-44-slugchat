...
// for OAUTH signup
User.handleOAUTH = function(profile) {
  if(!profile)
    return Promise.reject(
      createError(400, 'VALIDATION ERROR: no profile received!')
    );
  if(!profile.email)
    return Promise.reject(
      createError(400, 'VALIDATION ERROR: no email address received with profile!')
    );
  return User.findOne({email: profile.email}) //check if user exists
    .then(user => {
      if(!user)
    ...
    .catch(() => {
      console.log('Creating new user with GPlus profile data.');
      return new User({
