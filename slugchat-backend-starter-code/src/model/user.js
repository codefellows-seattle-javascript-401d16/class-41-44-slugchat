...
userSchema.methods.passwordCompare = function(password) {
  return bcrypt.compare(password, this.passwordHash).then(success => {
    if (!success) throw createError(401, "AUTH ERROR: wrong password");
    return this;
  });
};
...
User.createFromSignup = function(user) {
  ...
  return bcrypt.hash(password, 1).then(passwordHash => {
...
User.handleOAUTH = function(data) {
  ...
  return User.findOne({ email: data.email })
    .then(user => {
      if (!user) throw new Error("must create the user");
